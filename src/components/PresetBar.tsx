            ...
            return (
              ...
              >
                <button
                  ...
                  title={t("preset.apply", { name: p.name })}
                >
                  ...
                  <span className="flex shrink-0 items-center gap-1.5 text-[10px]">
                    {target > 0 && (
                      <span className="text-emerald-300">{t("preset.targetN", { n: target })}</span>
                    )}
                    {exclude > 0 && (
                      <span className="text-rose-300">{t("preset.excludeN", { n: exclude })}</span>
                    )}
                  </span>
                </button>
                <ConfirmButton
                  onConfirm={() => onDelete(p.id)}
                  label={t("preset.deleteName", { name: p.name })}
                  idle={<X size={13} />}
      ...
      {naming ? (
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            aria-label={t("preset.cancelSave")}
        ...
        <button
          ...
          disabled={!canSave}
          title={canSave ? t("preset.saveCurrent") : t("preset.noneToSave")}
