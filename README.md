...
候補になる枠の組み合わせを、次の優先順位で比較します（4/5 は既定の「リンク効果優先」の順序）。

1. 選択した目標属性のうち、結果に含まれる（Lv1 以上になる）属性の数
2. 選択した目標属性が Lv6 に到達した数
3. 全属性の Lv6 数
4. リンク効果（属性値の合計。ソフト除外を指定した属性の値は含めない）
5. 全属性の Lv5 数
6. 除外属性値の合計の小ささ

除外を指定していない場合、4 のリンク効果はゲーム内表示の「リンク効果」と同じ値です。

Lv6 数が同点のときは、リンク効果（属性値の合計）の大小を Lv5 数より先に見ます。Lv5
到達がもう1個あるだけでリンク効果の大差がひっくり返る、という直感に反した結果を避けるためです。

サイドバーの「並び順」で 4 と 5 を入れ替えた **Lv5 数優先** の順序にも切り替えられます
（Lv6 数が同点のとき、リンク効果より Lv5 到達数を優先したい場合向け）。**並び順を
切り替えただけでは自動的に再検索されないため、変更後は「最適化を実行」を押し直してください。**
2 つの順序は一般に互いの1位を再現できない（片方の順序で上位に出ない組み合わせが、もう
片方では1位になることがある）ため、見たい順序ごとに検索し直す設計です。

属性レベルは、属性値の合計が `1 / 4 / 8 / 12 / 16 / 20` に到達するごとに Lv1〜Lv6 として扱います。

- 目標属性は「含められるなら含める」ソフトな優先です。含められない場合は無視されます（下限 Lv を指定した属性のみ、満たさない組み合わせが除外される必須条件になります）。
- 除外属性は既定では評価から外すだけの**ソフト除外**で、該当属性を持つモジュール自体は採用されえます（その場合も除外属性値の合計が小さい組み合わせを優先）。「除外を完全排除する（ハード除外）」を有効にすると、除外属性を含むモジュールが候補から外れます。
- カードの大きい数字は常にゲーム内表示と一致する実際のリンク効果です。ソフト除外を指定していて評価に使う 4 の値と差がある場合のみ、下に小さく「評価スコア」として併記されます。
