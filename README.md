...
所持しているモジュールの中から、**狙った属性が一番伸びる 4/5 枠の組み合わせ**を総当たりで探し出します。所持モジュールはゲーム通信から自動で読み取るため、手入力は不要です。**外部サーバへのデータ送信は一切ありません。**
...
- **4/5 枠の全探索** — スロット数を 4 と 5 で切り替え可能。指定した枠数の組み合わせをすべて比較し、最良のセットを提示
...
- **お気に入りビルド** — 気に入った構成を ★ で保存。名前を付けて管理でき、複数ビルドを並べて比較可能
